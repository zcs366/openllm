...
"""Tr测量实验v2：修复检索精度问题
修复点：
1. 中文分词从bigram改为jieba
2. 添加同义词扩展（选/选择/决定/定了等）
3. top_k从6提高到12
4. 添加主题级别预过滤
"""
...
def _build_synonym_map():
    """构建同义词映射：word -> canonical word"""
    m={}
    for group in _SYNONYM_GROUPS:
        canonical=min(group)  # 用最短的词作为规范形式
        for w in group:
            m[w]=canonical
    return m

_SYN_MAP=_build_synonym_map()

def tok(t:str)->Set[str]:
    """分词：jieba中文 + 英文单词 + 同义词归一化"""
    jieba=_get_jieba()
    tokens=set()
    
    # 英文单词
    tokens.update(w.lower() for w in re.findall(r'[a-zA-Z_]{2,}',t.lower()))
    ...
    if jieba:
        for w in jieba.cut(t):
            ...
            if len(w)>=2 and w not in _STOP:
                # 同义词归一化
                tokens.add(_SYN_MAP.get(w,w))
    else:
        # fallback: bigram
        ch=[c for c in t if '\u4e00'<='\u9fff'<=c]
        tokens.update(ch[i]+ch[i+1] for i in range(len(ch)-1))
...
def topic_match(q_tokens:Set[str], stmt_tokens:Set[str])->bool:
    """主题级别匹配：如果query的关键词在statement中有≥2个命中，认为主题相关"""
    # 取query中较长的token（更可能是主题词）
    q_major={t for t in q_tokens if len(t)>=2}
    s_major={t for t in stmt_tokens if len(t)>=2}
    overlap=len(q_major&s_major)
    return overlap>=2

class Store:
    def __init__(self,budget=600,top_k=12):
        self.db:List[dict]=[]
        self.budget=budget
        self.top_k=top_k
    ...
    def query(self,q:str)->List[dict]:
        ...
        for m in self.db:
            base_sim=sim(qt,m["t"])
            # 主题匹配加分
            bonus=0.15 if topic_match(qt,m["t"]) else 0
            score=base_sim*0.7+bonus+0.15
            scored.append((score,m))
        
        scored.sort(key=lambda x:-x[0])
        sel,used=[],0
        for sc,m in scored:
            if len(sel)>=self.top_k or sc<0.05:
                break
            if used+len(m["c"])//2>self.budget:
                continue
            sel.append(m)
            used+=len(m["c"])//2
        return sel
...
def run():
    ...
    with open("tr_report_v2.json","w",encoding="utf-8") as f:
    ...
    print(f"报告: tr_report_v2.json");print("="*60)
    return report
