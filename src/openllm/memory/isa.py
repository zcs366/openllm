"""
[DEPRECATED — 2026-07-26] ISA v2.0 编排器已停用
═══════════════════════════════════════════════════
此文件定义的 ISA 类（381行）是8层循环的总装编排器。
当前无任何文件 import 此类（grep零引用确认）。
子模块（causal_memory/dashboard/immune/metacognition等）被其他文件独立使用。

保留原因：架构蓝图价值——未来重构ISA时可参考此8层循环设计。
处理方式：文件保留但不加载，零运行时开销。
═══════════════════════════════════════════════════
